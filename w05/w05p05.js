...
async function main() {
    ...
    // Extract material data and create corresponding buffers
    const { materialColorBuffer, materialEmissionBuffer, matIndicesBuffer, lightIndicesBuffer } = createMaterialBuffers(device, drawingInfo);
...
function createMaterialBuffers(device, drawingInfo) {
    ...
    const lightIndicesBuffer = createBuffer(device, drawingInfo.light_indices, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST);

    return { materialColorBuffer, materialEmissionBuffer, matIndicesBuffer, lightIndicesBuffer };
}
