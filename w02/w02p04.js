...
/**
 * Initializes WebGPU, sets up the rendering pipeline, and handles user interactions.
 */
async function main() {
    ...
    /**
     * IMPORTANT:
     * Ensure that the size of the uniform buffer matches the Uniforms struct in WGSL.
     * In the WGSL shader, Uniforms has three f32 values: aspect, cam_constant, shader.
     * Each f32 is 4 bytes, so the total size should be 12 bytes.
     */
    ...
    let zoom = 1.0;      // Initial zoom level (mapped to cam_constant)
